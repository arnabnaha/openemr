...
if ($_POST["action"]!="submit" ) {
	die ("You should send the document directly from the input form!.");
}//eof main check
...
$file_ext=$_FILES['document_image']['name'];
$extension=substr ( $file_ext , -4);
$file_new_name.=$extension;
// we check for a valid type of file.
if (($_FILES['document_image']['type'] == 'image/gif') || 
	($_FILES['document_image']['type'] == 'image/jpg') ||
	($_FILES['document_image']['type'] == 'application/pdf') ||
	($_FILES['document_image']['type'] == 'image/pjpeg') ||
	($_FILES['document_image']['type'] == 'image/jpeg') ||
	($_FILES['document_image']['type'] == 'image/png') ||
	($_FILES['document_image']['type'] == 'image/tiff') ||
	($_FILES['document_image']['type'] == 'application/x-vnd.oasis.opendocument.text') ||
	($_FILES['document_image']['type'] == 'text/richtext') ||
	($_FILES['document_image']['type'] == 'text/html') ||
	($_FILES['document_image']['type'] == 'application/msword') ||
	($_FILES['document_image']['type'] == 'audio/mpeg') ||
	($_FILES['document_image']['type'] == 'audio/x-wav') ||
	($_FILES['document_image']['type'] == 'video/mpeg') ||
	($_FILES['document_image']['type'] == 'application/vnd.sun.xml.writer') ||
	($_FILES['document_image']['type'] == 'image/bmp')){ 
	$checktype='ok';
}

if ($checktype!='ok') {
	echo ("<br><span class=text>That is not an accepted file type.  Types allowed are .odt, .pdf, .png, .jpg, .pjpg, .gif, .tiff, .bmp, .html, .doc, .mpg, .wav and .rtf.<br></span>");
	die ();
}
...
if (!is_file("$document_path/".$file_new_name) ) {
	if ( copy($_FILES['document_image']['tmp_name'], "$document_path/".$file_new_name ) ) {
